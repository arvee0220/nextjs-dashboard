...
export async function fetchCardData() {
    ...
    `;
        ...
        const numberOfInvoices = Number(result.invoice_count ?? "0");
        const numberOfCustomers = Number(result.customer_count ?? "0");
        const totalPaidInvoices = formatCurrency(result.total_paid ?? "0");
        const totalPendingInvoices = formatCurrency(result.total_pending ?? "0");
